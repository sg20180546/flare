    ...
    def __init__(self, args, rank, x_display=constants.X_DISPLAY,
        ...
        if args.use_learned_depth:
            ...
            if args.use_depth_anything:
                from models.depth.depth_models.metric_depth.zoedepth.models.builder import build_model
                from models.depth.depth_models.metric_depth.zoedepth.utils.config import get_config
                # Load default pretrained resource defined in config if not set
                pwd = os.getcwd()
                # overwrite = {"pretrained_resource": f'local::{pwd}/models/depth/depth_models/metric_depth/depth_anything_finetune/ZoeDepthv1_25-Jul_16-14-67e915e8fa2f_best.pt'}
                overwrite = {"pretrained_resource": f'local::/home/user/Depth-Anything/metric_depth/depth_anything_finetune/ZoeDepthv1_29-Jul_16-13-f06b635ecaab_best.pt'}
                config = get_config('zoedepth', "eval", 'alfred', **overwrite)
                self.depth_pred_model = build_model(config)
                print("depth initialized with Depth Anything")
...
###########################################################
        # if len(target_instance_id)>0 :
        #     cv2.imshow("GTmask",(self.event.events[0].instance_masks[target_instance_id]*255).astype(np.uint8) )
        #     cv2.waitKey(1)
###########################################################
        if debug:
