...
def main():
    ...
    for e in range(num_scenes):
        if actions_dicts[e] == 'GG':
            ...
            #Add to analyze recs 
            analyze_dict = {'task_type': None, 'errs':None, 'action_pointer':None, 'goal_found':None,\
            ...
            while True:
                if actions_dicts[e] == 'GG':
                    print("LLM gaved up")
                    actseq = {(number_of_this_episode, actions_dicts[e]): [{'action': 'LookDown_15', 'forceAction': True}]}
                    actseqs.append(actseq)
                    analyze_dict = {'task_type': None, 'errs':None, 'action_pointer':None, 'goal_found':None,\
                        'number_of_this_episode': number_of_this_episode}
                    if not(args.test):
    ...
    for e in range(num_scenes):
        if args.use_sem_policy:
            c1 = 120 # np.random.choice(local_w)
            c2 = 120 # np.random.choice(local_h)
        else:
            c1 = 120 # np.random.choice(local_w)
            c2 = 120 # np.random.choice(local_h)
    ...
    for e, p_input in enumerate(planner_inputs):
        ...
        p_input['class_map'] =local_map[e, 4:, :, :].cpu().numpy()
        if args.visualize or args.print_images:
            local_map[e, -1, :, :] = 1e-5
            p_input['sem_map_pred'] = local_map[e, 4:, :, :].argmax(0).cpu().numpy()

    obs, rew, done, infos, goal_success_s, next_step_dict_s,planner_inputs = envs.plan_act_and_preprocess(planner_inputs, goal_spotted_s)
    for e, p_input in enumerate(planner_inputs):
        list_of_actions_s[e] =  p_input['list_of_actions']
    ...
    for step in range(args.num_training_frames//args.num_processes):
        ...
        for e,x in enumerate(task_finish):
            if x:
                ...
                if not(finished[e]):
                    ...
                    ep_unity = _thor_env_module._unity_total_time
                    ...
                    _thor_env_module.reset_unity_timer()
            ...
            if args.delete_from_map_after_move_until_visible and (next_step_dict_s[e]['move_until_visible_cycled'] or next_step_dict_s[e]['delete_lamp']):
                ...
                for lab in unique_labels:
                    ...
                    if ((wheres[0] == 239) * (wheres[1] == 239)).any():
                        continue
                    center = (int(np.nanmean(wheres[0])), int(np.nanmean(wheres[1]))) ####np.mean ->np.nanmean
        ...
        if l_step == args.num_local_steps - 1:
            ...
            for e in range(num_scenes):
                ...
                else: # IS NOT use_sem_plolicy
                    # print('here')
                    x_240 = np.random.choice(local_w)
                    y_240 = np.random.choice(local_h)
                    global_goals[e] = [x_240, y_240]
        # ------------------------------------------------------------------
        ...
        ###################################      
        ###################################
        # print(planner_inputs[0]['list_of_actions'][planner_inputs[0]['list_of_actions_pointer']])
        obs, rew, done, infos, goal_success_s, next_step_dict_s,planner_inputs = envs.plan_act_and_preprocess(planner_inputs, goal_spotted_s)
        for e, p_input in enumerate(planner_inputs):
            list_of_actions_s[e] = p_input['list_of_actions']
        ...
        for e, p_input in enumerate(planner_inputs):
            # list_of_actions_s[e] = p_input['list_of_actions']
            # pointer = p_input['list_of_actions_pointer']
            # for n, x in enumerate(list_of_actions_s[e][pointer]):
            #     if x=='TrashCan':
            #         p_input['list_of_actions'][pointer][n]='GarbageCan'
            #         print('argument swap')
            #         goal_name = list_of_actions_s[e][list_of_actions_pointer_s[e]][0]
            #         reset_goal_true_false = [False] * num_scenes
            #         reset_goal_true_false[e] = True
        ...
        for e in range(num_scenes):
            number_of_this_episode = args.from_idx + traj_number[e] * num_scenes + e
            if task_finish[e] and not(finished[e]) and not(number_of_this_episode in skip_indices) and not LLM_giveup[e]:
                f = open("results/logs/log_" + args.eval_split + "_from_" + str(args.from_idx) + "_to_" + str(args.to_idx) + "_" + dn +".txt" , "a")
                number_of_this_episode = args.from_idx + traj_number[e] * num_scenes + e
                f.write("\n")
                f.write("===================================================\n")
    ...
    total_unity = _thor_env_module._unity_total_time
